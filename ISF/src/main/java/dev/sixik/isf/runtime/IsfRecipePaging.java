 ...
 * элементов, сколько помещается в фиксированную область просмотра.
 */
public final class IsfRecipePaging {
     ...
     * @return страницы-группы индексов; рецепт выше области получает собственную страницу
     */
    public static List<List<Integer>> partitionByHeight(List<Float> heights,
        ...
        if (heights == null || heights.isEmpty() || areaHeight <= 0.0f) {
            if (heights != null && !heights.isEmpty()) pages.add(List.copyOf(indexRange(heights.size())));
            return pages;
        }
        ...
        for (int index = 0; index < heights.size(); index++) {
            ...
            if (!current.isEmpty() && used + extra + height > areaHeight) {
